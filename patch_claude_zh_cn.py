...
def patch_hardcoded_frontend_strings(app: Path) -> None:
    ...
    replacements = {
        ...
        '"\\"Essential\\" means the signals Anthropic needs to keep your deployment working: **crash stacks**, **startup failure reasons**, and **version/OS metadata**. No prompts, completions, file contents, or identifiers beyond a random install ID.\n\n**What you lose when this is on:** when a Cowork build hits a bug that only reproduces on your OS version or locale, Anthropic can\'t see it unless a user manually reports. Fixes ship slower.\n\n**Why this is discouraged, not blocked:** some air-gapped environments require zero outbound telemetry as a matter of policy. The switch exists for them — if you don\'t have that constraint, leave it off."': '"“基本”是指 Anthropic 维持部署正常运行所需的信号：**崩溃堆栈**、**启动失败原因**和**版本/操作系统元数据**。除了随机安装 ID 之外，不会收集任何提示、补全信息、文件内容或标识符。\n\n**启用此功能后会丢失什么：**当 Cowork 构建遇到仅在您的操作系统版本或语言环境中重现的错误时，除非用户手动报告，否则 Anthropic 将无法检测到该错误。修复程序的发布速度会变慢。\n\n**不建议启用此功能，但不会完全禁用：**某些物理隔离环境出于策略考虑，要求完全禁止出站遥测。您可以为此设置开关——如果您没有此类限制，请将其关闭。"',
        ...
        '{connection:{title:"Connection",description:"Choose where Claude Desktop sends inference requests."},sandbox:{title:"Sandbox & workspace"},connectors:{title:"Connectors & extensions"},telemetry:{title:"Telemetry & updates",banner:"Prompts, completions, and your data are never sent to Anthropic — telemetry covers crash and usage signals only."},limits:{title:"Usage limits"},plugins:{title:"Plugins & skills",banner:"Plugins and skills aren\'t set in this configuration. Mount plugin bundles to the folder below using your device-management tool and Cowork will load them at launch.",mountPath:{mac:"/Library/Application Support/Claude/org-plugins",win:"%ProgramFiles%\\\\Claude\\\\org-plugins",caption:"Drop plugin folders here. Read-only to the app."}},egress:{title:"Egress Requirements",description:"Hosts your network firewall must allow, derived from your current settings. This list is read-only and updates as you make changes. Traffic is HTTPS on port 443 unless a custom port is specified (OTLP, gateway, or MCP server URLs).",': '{connection:{title:"连接",description:"选择 Claude Desktop 向何处发送推理请求。"},sandbox:{title:"沙盒和工作区"},connectors:{title:"连接器和扩展"},telemetry:{title:"遥测和更新",banner:"提示、完成情况和您的数据永远不会发送给 Anthropic——遥测数据仅涵盖崩溃和使用情况信号。"},limits:{title:"使用限制"},plugins:{title:"插件和技能",banner:"此配置中未设置插件和技能。请使用设备管理工具将插件包挂载到以下文件夹，Cowork 将在启动时加载它们。",mountPath:{mac:"/Library/Application Support/Claude/org-plugins",win:"%ProgramFiles%\\\\Claude\\\\org-plugins",caption:"将插件文件夹拖放到此处。对应用程序只读。"}},egress:{title:"出口要求",description:"此列表包含您的网络防火墙必须允许的主机，其值取决于您当前的设置。此列表为只读，会随着您的更改而更新。除非指定自定义端口（例如 OTLP、网关或 MCP 服务器 URL），否则流量将通过 443 端口上的 HTTPS 协议传输。",',
